namespace FaceDetect.FormsApp.Components.Device
{
    using System;
        ...
        private static NetworkState GetNetworkState(NetworkAccess access, IEnumerable<ConnectionProfile> profiles)
        {
            if (access != NetworkAccess.None && access != NetworkAccess.Unknown)
            {
                return profiles.Any(x => x == ConnectionProfile.Ethernet || x == ConnectionProfile.WiFi)
                    ? FaceDetect.FormsApp.Components.Device.NetworkState.ConnectedHighSpeed
                    : FaceDetect.FormsApp.Components.Device.NetworkState.Connected;
            }

            return FaceDetect.FormsApp.Components.Device.NetworkState.Disconnected;
        }
